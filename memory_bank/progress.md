...
## P5 sweep harness + P4 observer scaffold (2026-07-03, soak deferred to overnight)

- User call: defer the soak to overnight; advance other phases during the day. Priority: sweep harness first (it converts the soak into the experiment), then observer.
- **experiments/sweep.py** (new `experiments/` package): base experiment_config fans out over N seeds as cognition.runner subprocesses, round-robin across serving profiles with per-profile concurrency caps; per-seed provenance (derived config with assigned profile, auto-filled roles, real content_hash) written before launch; bundles + cost.json per seed; sweep-root summary.json + cross-seed diffusion overlay plot; resume skips complete seeds. cognition.runner --config now applies Settings (thresholds/retrieval/perception) — the config is the experiment, fully.
- **observer/ scaffolded** (Vite + React + R3F + Zustand + Ajv, pnpm + vitest): drag-drop ledger.jsonl or live `?ws=` stream; every event Ajv-validated against the shared schema; pure replay fold (world-at-tick = fold of prefix) drives a tick scrubber with play; Harrowmere rendered as simple geometry from town.json; agents as status-colored labeled spheres, click → inspector (state + event feed). 6 vitest tests run against the real P1 byte-equal fixture. Remaining: memories.jsonl inspector, diffusion overlay, relationship threads, live gate check.
- Suites: 152 pytest green + 6 vitest green.

## P4 observer feature-complete for software (2026-07-03, evening)

- All remaining P4 software boxes closed in one pass: **memories inspector** (drop memories.jsonl; click an agent → their memory stream as of the cursor), **diffusion overlay** (seeded fact read from treatment_injected; holders haloed via a TS port of probes.py's STOPWORDS + 60%-overlap rule, parity-tested; updates live while scrubbing), **relationship threads** (pair lines weighted by accumulated conversation; active pairs glow amber), **day/night lighting** (pure function of sim time — scrub to dawn, get dawn), occluders rendered, smoothed agent movement, multi-file drop routed by record shape.
- Demo dataset `runs/demo_treated_fake/` (fake-model treated day, non-conforming): fact diffuses maren → ilse+piet, Cranes never reached — the P3 curve, now watchable.
- 14 vitest tests (P1 fixture contract, fold purity, fact-check parity, edge fold, treatment extraction). Remaining P4: the live-session gate check only.
- **Live mode wired** (same evening): `cognition.runner --serve-ws PORT` starts the sidecar in-process; LedgerWriter on_event fans out to Postgres + broadcaster; LedgerBroadcaster now keeps history so mid-run subscribers get the backlog first (seq-deduped at the register/snapshot race), then live-tail. `--tick-seconds` gives wall-clock viewing pace without touching sim semantics. Observer appendLive folds incrementally (no O(n²) refold). Found by smoke-testing the real socket: bare uvicorn has no WS protocol impl — `websockets` added to deps (TestClient had masked it). 154 pytest + 14 vitest green.
